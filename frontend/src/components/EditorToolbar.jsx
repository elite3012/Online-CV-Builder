...
export default function EditorToolbar({
  onBack, cvTitle, setCvTitle, isEditingTitle, setIsEditingTitle, 
  templateName, saveStatus, onPreview, onExport
}) {
  return (
    <Paper elevation={1} sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", px: 3, py: 1.5, zIndex: 10, borderRadius: 0 }}>
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        ...
        {isEditingTitle ? (
          <TextField size="small" value={cvTitle} autoFocus onChange={(e) => setCvTitle(e.target.value)} onBlur={() => setIsEditingTitle(false)} onKeyDown={(e) => e.key === "Enter" && setIsEditingTitle(false)} sx={{ width: 150 }} />
        ) : (
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        <Typography variant="body2" color="text.secondary">Template: <b>{templateName || "Unknown"}</b></Typography>
        ...
        <Button variant="outlined" size="small" onClick={onPreview} sx={{ color: "#102a43", borderColor: "#e0e0e0", textTransform: "none" }}>Preview</Button>
        <Button variant="contained" size="small" onClick={onExport} sx={{ bgcolor: "#52b0c3", "&:hover": { bgcolor: "#3d94a7" }, textTransform: "none" }}>Export</Button>
      </Box>
    </Paper>
  );
}