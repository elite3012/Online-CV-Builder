...
export default function Editor({ template: propTemplate, onBack: propOnBack }) {
  ...
  const previewData = {
    ...
    experience: formData.experience.filter((e) => e.company).map((exp) => ({ company: exp.company, role: exp.role, duration: `${exp.startDate} - ${exp.endDate}`, desc: exp.description })),
    skills: formData.skills ? formData.skills.split(",").map((s) => s.trim()) : [],
    education: formData.education.filter((e) => e.school).map((edu) => ({ school: edu.school, degree: edu.degree, duration: `${edu.startDate} - ${edu.endDate}`, desc: edu.description })),
    ...
    <Box sx={{ display: "flex", flexDirection: "column", height: "100vh", bgcolor: "#f8f9fb" }}>
      ...
      <Box sx={{ display: "flex", flexGrow: 1, overflow: "hidden" }}>
        ...
        <Box sx={{ flexGrow: 1, p: 5, overflowY: "scroll", display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          {activeSection === "Education" && renderArraySection("Education", "education", "Education", { school: "", degree: "", startDate: "", endDate: "", description: "" }, (edu, idx) => (
          ...
          {activeSection === "Experience" && renderArraySection("Work Experience", "experience", "Job", { company: "", role: "", startDate: "", endDate: "", description: "" }, (exp, idx) => (
          ...
          {activeSection === "Projects" && renderArraySection("Project", "projects", "Project", { name: "", role: "", link: "", description: "" }, (proj, idx) => (
          ...
          {activeSection === "Certificates" && renderArraySection("Certificate", "certificates", "Certificate", { name: "", organization: "", date: "" }, (cert, idx) => (
